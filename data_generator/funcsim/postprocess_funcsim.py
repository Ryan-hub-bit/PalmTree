...
4. Save common function names to a log file for each package
...
def save_common_functions(folder_path, common_functions):
    """Save common function names to a log file."""
    if not common_functions:
        return
    
    project_name = os.path.basename(folder_path)
    log_file = os.path.join(folder_path, f"{project_name}_common_functions.txt")
    
    with open(log_file, 'w') as f:
        f.write(f"# Common functions found in ALL JSON files in {folder_path}\n")
        f.write(f"# Total: {len(common_functions)} functions\n")
        f.write(f"# Generated by postprocess_funcsim.py\n")
        f.write("#\n")
        for func_name in sorted(common_functions):
            f.write(f"{func_name}\n")
    
    print(f"  Saved common function names to: {log_file}")
...
def process_folder(folder_path):
    ...
    if len(all_data) >= 2:
        ...
        if common_functions:
            ...
            for func in sorted(list(common_functions))[:20]:
                print(f"    - {func}")
            # Save common functions to file
            save_common_functions(folder_path, common_functions)
    else:
    ...
    print("Summary:")
    print("=" * 90)
    print(f"{'JSON File':<35} {'Original':>10} {'Incomplete':>12} {'Short(<10)':>12} {'Common':>10} {'Final':>10}")
    print("-" * 90)
