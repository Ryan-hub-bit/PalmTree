#!/bin/bash
#
# Complete pipeline for addressaware dataset generation (pretrain + finetune)
# 
# Usage: ./generate_addressaware_dataset.sh <stripped_dir> <nonstripped_dir> <output_dir>
#
# Example:
#   ./generate_addressaware_dataset.sh \
#     /data/kun/jtrans/small_train_strip \
#     /data/kun/jtrans/small_train \
#     /data/kun/jtrans/addressaware
#
# Three steps:
#   1. function_export_ida.py - IDA Pro extracts functions from stripped → pickle files
#   2. combine_function_files_pretrain.py - Combine pickles → addr_pretrain.txt
#   3. create_function_dataset.py - Generate JSON (uses nonstripped for function names)
#
...
# Check arguments
if [ "$#" -lt 3 ]; then
    echo "Usage: $0 <stripped_dir> <nonstripped_dir> <output_dir>"
    echo ""
    echo "Arguments:"
    echo "  stripped_dir    : Directory containing stripped binaries (for IDA extraction)"
    echo "  nonstripped_dir : Directory containing original binaries (for function names)"
    echo "  output_dir      : Output directory"
    echo ""
    echo "Example:"
    echo "  $0 /data/kun/jtrans/small_train_strip \\"
    echo "     /data/kun/jtrans/small_train \\"
    echo "     /data/kun/jtrans/addressaware"
    echo ""
    echo "This script will generate:"
    echo "  1. Pickle files (from stripped binaries)"
    echo "  2. addr_pretrain.txt (for pretraining)"
    echo "  3. func_blocks_addr.json + ground_truth_addr.json (uses nonstripped for function names)"
    exit 1
fi
...
echo "========================================================================"
echo "         ADDRESSAWARE COMPLETE DATASET GENERATION"
echo "========================================================================"
echo ""
echo "Configuration:"
...
echo "  - Pickle files (from stripped binaries)"
echo "  - addr_pretrain.txt (pretraining)"
echo "  - func_blocks_addr.json (finetuning)"
echo "  - ground_truth_addr.json (evaluation - uses function names from nonstripped)"
echo ""
echo "========================================================================"
echo ""
...
find "$STRIPPED_DIR" -maxdepth 1 -type f ! -name "*.i64" ! -name "*.id0" ! -name "*.id1" ! -name "*.id2" ! -name "*.nam" ! -name "*.til" | while read -r binary; do
    ...
    # Run IDA Pro in batch mode (show progress, save logs)
...
# ============================================================================
# STEP 2: Combine pickle files into pretrain format
# ============================================================================
...
if python3 "$COMBINE_SCRIPT" \
    ...
    if [ -f "$OUTPUT_DIR/top_symbols.txt" ]; then
        ...
        echo "  - $OUTPUT_DIR/top_symbols.txt ($symbol_count symbols)"
    fi
else
    print_error "Failed to combine pickle files"
    exit 1
fi
echo ""
...
echo ""
echo "========================================================================"
print_success "COMPLETE DATASET GENERATION FINISHED"
echo "========================================================================"
echo ""
echo "Generated files in: $OUTPUT_DIR"
echo ""
echo "Pretraining:"
echo "  - addr_pretrain.txt"
...
echo ""
echo "Next steps:"
echo "  1. Pretrain: cd /home/kun/Document/AAE/strupos && ./run.sh"
echo "  2. Finetune: cd /home/kun/Document/AAE/extern/jTrans && ./run_finetune_addressaware.sh"
echo "  3. Evaluate: python3 evaluate_addressaware_with_pools.py"
echo ""
echo "========================================================================"
