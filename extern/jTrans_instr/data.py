...
import random
import time
MAXLEN=200  # Changed from 512 to 200 for instruction-level addressing
...
def gen_funcstr(f,convert_jump):
    cfg=f[3]
    #print(hex(f[0]))
    bb_ls,code_lst,map_id,map_instr_id=[],[],{},{}
    instr_count = 0
    for bb in cfg.nodes:
        bb_ls.append(bb)
    bb_ls.sort()
    for bx in range(len(bb_ls)):
        ...
        map_id[bb]=len(code_lst)  # Map BB to token position (for compatibility)
        map_instr_id[bb]=instr_count  # Map BB to instruction index
        for code in asm:
            ...
            instr_count += 1  # Count instructions, not tokens
    for c in range(len(code_lst)):
        op=code_lst[c]
        if op.startswith('hex_'):
            jumpaddr=int(op[4:],base=16)
            if map_instr_id.get(jumpaddr) is not None:  # Use instruction index
                instr_idx=map_instr_id[jumpaddr]
                if instr_idx < MAXLEN:
                    code_lst[c]='instr_addr_{}'.format(instr_idx)
                else:
                    code_lst[c]='instr_addr_exceed'
            else:
                code_lst[c]='unknown_instr_addr'
            if not convert_jump:
                code_lst[c]='CONST'
