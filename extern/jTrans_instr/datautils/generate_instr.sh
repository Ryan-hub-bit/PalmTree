...
echo "Output: $OUTPUT_DIR/instr_pretrain.txt"
echo ""

python3 convert_pkl_to_text.py \
    "$EXTRACT_DIR" \
    "$OUTPUT_DIR/instr_pretrain.txt" \
    --min-instructions 5 \
...
if [ -f "$OUTPUT_DIR/instr_pretrain.txt" ]; then
    line_count=$(wc -l < "$OUTPUT_DIR/instr_pretrain.txt")
    echo "Generated $line_count functions in instr_pretrain.txt"
fi

echo ""
echo "========================================================================"
echo "Step 4/4: Generating instruction-level function dataset"
echo "========================================================================"
...
echo "  - $OUTPUT_DIR/instr_pretrain.txt"
echo "  - $OUTPUT_DIR/func_blocks_instr.json"
echo "  - $OUTPUT_DIR/ground_truth_instr.json"
echo ""
echo "Next steps:"
echo "  1. Build vocabulary: python create_vocab.py --input_file instr_pretrain.txt"
echo "  2. Run instruction-level pretraining"
echo "  3. Run instruction-level finetuning"
echo ""
