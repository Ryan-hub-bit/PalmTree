#!/usr/bin/env python3
"""
Convert pickle files from IDA extraction to text format for jTrans_instr pretraining.

Implements instruction-level jump addressing:
- For each jump instruction, replace target address with instr_addr_{i}
- Where i is the 0-indexed instruction index (not token position) in the function
- Uses CFG to accurately resolve jump targets

Input: *_extract.pkl files 
Output: Text file with one function per line, tokenized assembly with instr_addr_{i}.
"""
...
def convert_function_to_text_with_jumps(func_data):
    """
    Convert function to text with instruction-level jump addressing (instr_addr_{i}).
    
    This uses instruction indices (not token positions):
    1. Identify all jump instructions
    2. Use CFG to find their target instruction indices
    ...
    for i, asm_str in enumerate(asm_list):
        try:
            ...
            if operator and operator.startswith('j'):
                ...
                if i in jump_map:
                    # Replace operand with instr_addr_{i} (instruction index)
                    target_instr_idx = jump_map[i]
                    if target_instr_idx < MAXLEN:
                        tokens.append(f'instr_addr_{target_instr_idx}')
                    else:
                        tokens.append('instr_addr_exceed')
                else:
                    ...
                    if op1 and (op1 == 'UNK_ADDR' or op1.startswith('hex_')):
                        ...
                        tokens.append('unknown_instr_addr')
                    elif op1:
                        tokens.append(op1)
    ...
    with open(output_path, 'a') as out:  # Append mode
        for func_name, func_data in data.items():
            ...
            if text:
                ...
                num_jumps = text.count('instr_addr_')
...
def main():
    ...
    print(f"\n📝 Sample with instr_addr_{{i}}:")
    with open(args.output_file, 'r') as f:
        for i, line in enumerate(f):
            if i >= 3:
                break
            if 'instr_addr_' in line:
                print(f"   {line[:120]}...")
                print(f"      → {line.count('instr_addr_')} jump(s)")
            else:
                print(f"   {line[:100]}...")
