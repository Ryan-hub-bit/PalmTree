"""
jTrans_instr Training Script for Pretraining

MLM + JTP (Jump-Target Prediction) with instruction-level embeddings.

Key differences from baseline jTrans:
1. Uses instruction embeddings instead of position=word trick
2. Jump addresses are instr_addr_{i} (instruction indices) not JUMP_ADDR_X (token positions)
3. Requires instruction_ids tensor mapping tokens to instructions
4. instr_addr_{i} tokens use instruction embedding i directly

Tasks:
1. MLM: Mask regular tokens (15%) and predict them
2. JTP: Mask instr_addr tokens (20%) and predict target instruction index

Usage:
    python train_instr.py \
        --train_path /path/to/instr_pretrain.txt \
        --test_path /path/to/instr_test.txt \
        --tokenizer_path /path/to/jtrans_tokenizer \
        --output_dir ./output_instr \
        --batch_size 32 \
        --learning_rate 1e-4 \
        --num_epochs 10
"""
...
if 'CUDA_VISIBLE_DEVICES' in os.environ:
    ...
    print("[WARNING] Example: bash run_instr_pretrain.sh")
    sys.exit(1)
...
from dataloader_instr import create_instr_dataloaders
...
def setup_logging(output_dir):
    ...
    log_file = os.path.join(output_dir, f'train_instr_{timestamp}.log')
...
def main():
    parser = argparse.ArgumentParser(description='jTrans_instr Pretraining (MLM + JTP with instruction embeddings)')
    
    # Data paths
    parser.add_argument('--train_path', type=str, required=True,
                       help='Path to training data (instr_pretrain.txt)')
    parser.add_argument('--test_path', type=str, default=None,
                       help='Path to test data (instr_test.txt). If not provided, validation will be skipped.')
    parser.add_argument('--tokenizer_path', type=str, required=True,
                       help='Path to tokenizer directory (jtrans_tokenizer with vocab.txt)')
    ...
    parser.add_argument('--max_instructions', type=int, default=201,
                       help='Maximum number of instructions (201 for indices 0-200)')
    ...
    parser.add_argument('--jtp_probability', type=float, default=0.20,
                       help='JTP masking probability for instr_addr tokens')
    
    # Data sampling
    parser.add_argument('--data_ratio', type=float, default=1.0,
                       help='Fraction of data to use (0.0001 = 0.01%%, 1.0 = 100%%)')
    
    # Output
    parser.add_argument('--output_dir', type=str, default='./output_instr',
    ...
    # Log configuration
    logger.info("=" * 80)
    logger.info("jTrans_instr Pretraining (MLM + JTP with Instruction Embeddings)")
    logger.info("=" * 80)
    ...
    logger.info(f"Data ratio: {args.data_ratio} ({args.data_ratio*100:.4f}%)")
    ...
    train_loader, test_loader = create_instr_dataloaders(
    ...
    if test_loader is not None:
        ...
        logger.info("No test data provided - validation will be skipped")
    
    # Create model
    logger.info("Creating jTrans_instr model with instruction embeddings...")
    ...
    logger.info("Setting instr_addr token IDs for special embedding handling...")
    model.set_instr_addr_token_ids(tokenizer)
    logger.info("✓ instr_addr_{i} tokens will use instruction_embedding[i]")
    ...
    for epoch in range(args.num_epochs):
        ...
        if test_loader is not None:
            ...
            # No validation
            val_loss = train_loss
            val_mlm_loss = train_mlm_loss
            val_jtp_loss = train_jtp_loss
            val_mlm_acc = train_mlm_acc
            val_jtp_acc = train_jtp_acc
        ...
        if (epoch + 1) % args.save_every == 0:
            ...
            training_info = {
                ...
                'architecture': {
                    ...
                    'vocab_size': vocab_size,
                    'max_instructions': args.max_instructions,
                    'has_instruction_embeddings': True
                }
            }
