...
class BERTDatasetAddressAware(Dataset):
    ...
    def parse_instruction_with_address(self, instruction):
        ...
        Returns:
            ...
            var_offsets: list of variable offsets (0 for non-var tokens)
        """
        tokens = []
        binary_positions = []
        function_positions = []
        ...
        for part in parts:
            ...
            if var_match:
                ...
                tokens.append('var')
                # For var tokens, use 0.0 for address positions (not applicable)
    ...
    def __getitem__(self, item):
        ...
        d1_tokens, d1_bpos, d1_fpos, d1_bbpos, d1_voffsets = self.parse_instruction_with_address(d1)
        d2_tokens, d2_bpos, d2_fpos, d2_bbpos, d2_voffsets = self.parse_instruction_with_address(d2)

        # Apply random masking to DFG
        d1_random, d1_label, d1_bpos, d1_fpos, d1_bbpos, d1_voffsets = self.random_word(
            d1_tokens, d1_bpos, d1_fpos, d1_bbpos, d1_voffsets
        )
        d2_random, d2_label, d2_bpos, d2_fpos, d2_bbpos, d2_voffsets = self.random_word(
            d2_tokens, d2_bpos, d2_fpos, d2_bbpos, d2_voffsets
        )
        ...
        d1_bpos = [0.0] + d1_bpos + [0.0]
        d1_fpos = [0.0] + d1_fpos + [0.0]
        d1_bbpos = [0.0] + d1_bbpos + [0.0]
        d1_voffsets = [0] + d1_voffsets + [0]
        
        d2_bpos = d2_bpos + [0.0]
        d2_fpos = d2_fpos + [0.0]
        d2_bbpos = d2_bbpos + [0.0]
        d2_voffsets = d2_voffsets + [0]

        # Parse CFG sequences with address info
        c1_tokens, c1_bpos, c1_fpos, c1_bbpos, c1_voffsets = self.parse_instruction_with_address(c1)
        c2_tokens, c2_bpos, c2_fpos, c2_bbpos, c2_voffsets = self.parse_instruction_with_address(c2)
        ...
        c1_bpos = [0.0] + c1_bpos + [0.0]
        c1_fpos = [0.0] + c1_fpos + [0.0]
        c1_bbpos = [0.0] + c1_bbpos + [0.0]
        c1_voffsets = [0] + c1_voffsets + [0]
        
        c2_bpos = c2_bpos + [0.0]
        c2_fpos = c2_fpos + [0.0]
        c2_bbpos = c2_bbpos + [0.0]
        c2_voffsets = c2_voffsets + [0]
        ...
        zero_padding = [0.0 for _ in range(self.seq_len - len(dfg_bert_input))]
        
        dfg_bert_input.extend(padding)
        dfg_bert_label.extend(padding)
        dfg_segment_label.extend(padding)
        dfg_binary_pos.extend(zero_padding)
        dfg_function_pos.extend(zero_padding)
        dfg_bb_pos.extend(zero_padding)
        dfg_var_offsets.extend(padding)
        
        cfg_padding = [self.vocab.pad_index for _ in range(self.seq_len - len(cfg_bert_input))]
        cfg_zero_padding = [0.0 for _ in range(self.seq_len - len(cfg_bert_input))]
        
        cfg_bert_input.extend(cfg_padding)
        cfg_segment_label.extend(cfg_padding)
        cfg_binary_pos.extend(cfg_zero_padding)
        cfg_function_pos.extend(cfg_zero_padding)
        cfg_bb_pos.extend(cfg_zero_padding)
        cfg_var_offsets.extend(cfg_padding)
    ...
    def random_word(self, tokens, binary_pos, function_pos, bb_pos, var_offsets):
        """
        ...
        for i, token in enumerate(tokens):
            ...
            if prob < 0.15:
                ...
                if prob < 0.8:
                    ...
                    # When masking, set address info to 0 (masked)
                    binary_pos[i] = 0.0
                    function_pos[i] = 0.0
                    bb_pos[i] = 0.0
                    var_offsets[i] = 0
        ...
        return masked_tokens, output_label, binary_pos, function_pos, bb_pos, var_offsets
