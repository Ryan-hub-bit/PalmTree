"""
Test script for BERT2 model (no address embeddings).
MUST match train_palmtree.py configuration exactly:
- Model: bert_pytorch.BERT2 (NO address embeddings)
- hidden=128, n_layers=6, attn_heads=8, dropout=0.1
- seq_len=100 (for 8 instructions)
- batch_size=32
- Dataset: BERTDataset with NO address files (all None)
"""
import torch
import numpy as np
from palmtree.trainer.pretrain import BERTTrainer
import bert_pytorch
from bert_pytorch import BERT2
...
def create_test_dataloader(data_path, vocab_path, batch_size=32, seq_len=100):
    """Create test dataloader for BERT2 model (no addresses)."""
    # Load vocabulary
    ...
    # Construct paths for instruction files (8-instruction format)
    cfg_dataset = os.path.join(data_path, "cfg_8.txt")
    dfg_dataset = os.path.join(data_path, "dfg_8.txt")
    
    # Verify required files exist
    verify_files_exist(cfg_dataset, dfg_dataset)
    
    print(f"Using sequence length: {seq_len}")
    print("Note: NO address files used (BERT2 model)")
    
    # Create test dataloader - matching train_palmtree.py with keyword arguments
    test_dataset = BERTDataset(
        cfg_corpus_path=cfg_dataset,
        dfg_corpus_path=dfg_dataset,
        cfg_src_path=None,  # No address files
        dfg_src_path=None,
        cfg_tgt_path=None,
        dfg_tgt_path=None,
        vocab=vocab,
    ...
    return torch.utils.data.DataLoader(test_dataset, batch_size=batch_size, num_workers=4)
...
def load_and_evaluate_model(model_path, test_data_path, vocab_path, batch_size=32, seq_len=100):
    ...
    try:
        ...
        # Create a new BERT2 model with the same architecture as used in training
        print("Initializing BERT2 model (no addresses)...")
        # IMPORTANT: Must match train_palmtree.py config exactly
        # BERT2, hidden=128, n_layers=6, attn_heads=8, dropout=0.1
        bert = bert_pytorch.BERT2(vocab_size, hidden=128, n_layers=6, attn_heads=8, dropout=0.1)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Evaluate BERT2 model (no addresses) on test data")
    ...
    try:
        ...
        # Find and verify checkpoints (transformer.ep* for noaddr model)
        checkpoints = sorted([f for f in os.listdir(args.model_dir) 
                            if f.startswith('transformer.ep')])
        if not checkpoints:
            raise FileNotFoundError(f"No transformer.ep* checkpoints found in {args.model_dir}")
        ...
        for checkpoint in progress_bar:
            ...
            try:
                ...
                desc = f"Epoch {epoch:02d} - Loss: {epoch_metrics['total_loss']:.4f}, DFG Acc: {epoch_metrics['dfg_nsp_acc']:.4f}, CFG Acc: {epoch_metrics['cfg_nsp_acc']:.4f}"
