...
def build_chunk_inline(seq, start, k, ctx: dict):
    ...
    def format_positions(entry):
        """Format positions for the opcode address (current instruction)."""
    ...
    def format_operand_positions(tgt_addr, curr_func_start, curr_func_end, 
                                  curr_bb_start_addr, curr_bb_end_addr):
        """
        Format operand positions using DISTANCE from current instruction's context.
        Uses clipping bounds: fnorm [-3, +3], bbnorm [-10, +10]
        """
        # Binary-level: still use absolute position
        if max_addr > min_addr:
            bnorm = (tgt_addr - min_addr) / float(max_addr - min_addr)
            bnorm = max(0.0, min(1.0, bnorm))
        else:
            bnorm = 0.0
        
        # Function-level: DISTANCE in function-relative units, clipped to [-3, +3]
        if curr_func_end > curr_func_start:
            func_size = float(curr_func_end - curr_func_start)
            # Distance in function-normalized units
            fnorm_dist = (tgt_addr - curr_func_start) / func_size
            # Clip to [-3, +3]
            fnorm = max(-3.0, min(3.0, fnorm_dist))
        else:
            fnorm = 0.0
        
        # Basic block-level: DISTANCE in bb-relative units, clipped to [-10, +10]
        if curr_bb_end_addr > curr_bb_start_addr:
            bb_size = float(curr_bb_end_addr - curr_bb_start_addr)
            # Distance in bb-normalized units
            bbnorm_dist = (tgt_addr - curr_bb_start_addr) / bb_size
            # Clip to [-10, +10]
            bbnorm = max(-10.0, min(10.0, bbnorm_dist))
        else:
            bbnorm = 0.0
    ...
    for addr, norm_line, mask_line in chunk:
        ...
        # Extract current instruction's context for operand distance calculation
        curr_func_start = 0
        curr_func_end = 0
        curr_bb_start_addr = 0
        curr_bb_end_addr = 0
        
        if addr in addr_positions:
            _, _, _, curr_bb_start, curr_func_start, curr_func_end = addr_positions[addr]
            if curr_bb_start in bb_range_map:
                curr_bb_start_addr, curr_bb_end_addr = bb_range_map[curr_bb_start]
            ...
            if mk_hex is not None:
                ...
                if tgt is not None and tgt >= min_addr and not is_immediate:
                    if tgt in addr_positions:
                        # Code address: use DISTANCE-based encoding with current context
                        pos = format_operand_positions(tgt, curr_func_start, curr_func_end,
                                                       curr_bb_start_addr, curr_bb_end_addr)
...
def process_file(f):
    ...
    import __main__
    out_dir_str = getattr(__main__, 'OUTPUT_DIR', '/home/kun/Document/PalmTree/data/dfg_2')
    ...
    code_min = min(all_addrs) if all_addrs else 0
    code_max = max(all_addrs) if all_addrs else 0
...
def main():
    ...
    out_dir = '/home/kun/Document/PalmTree/data/dfg_2'
