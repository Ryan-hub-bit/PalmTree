from binaryninja import load, BranchType
...
def process_file(fpath: str):
    ...
    func_entry_map = {}  # func_start_addr -> func
    call_sites = []      # List of (call_addr, call_inst, next_addr)
    bin_counter = 0

    print("[INFO] Building inter-procedural CFG...")
    
    # First pass: Build nodes and intra-procedural edges
    for func in bv.functions:
        func_start = func.start
        func_end = func.start + func.total_bytes
        func_entry_map[func_start] = func
        ...
        # Add nodes and intra-procedural edges
        for block in func:
            curr = block.start
            predecessor = curr
            for inst in block:
                ...
                # Check if this is a DIRECT call instruction (has hex address in disassembly)
                if disasm_raw.lower().startswith('call'):
                    # Only process direct calls (those with 0x... addresses)
                    hex_match = re.search(r'0x[0-9a-fA-F]+', disasm_raw)
                    if hex_match:
                        inst_len = inst[1]
                        next_addr = curr + inst_len  # Return address (next instruction)
    ...
    # Second pass: Add inter-procedural call and return edges
    print(f"[INFO] Adding {len(call_sites)} call/return edges...")
    call_edges_added = 0
    ...
    for call_addr, call_disasm, next_addr in call_sites:
        try:
            ...
            # Example formats: "call 0x401234", "call rax", "call qword [rip+0x1234]"
            call_target = None
            ...
            if call_target is not None and call_target in func_entry_map:
                ...
                called_func = func_entry_map[call_target]
                for block in called_func.basic_blocks:
                    ...
                    if last_inst_addr is not None:
                        ...
                        last_inst = bv.get_disassembly(last_inst_addr)
                        if last_inst.lower().startswith('ret'):
    ...
    print(f"[INFO] Added {call_edges_added} call edges and {return_edges_added} return edges")
    ...
    walks = random_walk(G, length=40, max_sequences=5000)
