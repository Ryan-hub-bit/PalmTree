...
Evaluates how well BERT embeddings encode position within a binary
by predicting which bucket (0-9) an instruction belongs to based on binary_norm.

Bucket assignment: bucket = int(binary_norm * 10)
where binary_norm is the normalized position within the binary [0.0, 1.0)
"""
...
class BinaryInstructionDataset(Dataset):
    """
    Dataset for instruction-level binary bucket prediction.
    Each instruction is labeled with its binary bucket (0-9 based on binary_norm).
    """
    ...
    def __init__(self, instructions, labels, vocab, seq_len=20):
        ...
        Args:
            ...
            labels: List of binary bucket labels (0-9)
...
def main():
    parser = argparse.ArgumentParser(description="Binary Bucket Probe")
    ...
    # Output
    parser.add_argument('--output', default='probe_results_binary', help='Output directory')
    ...
    for inst_file in instruction_files:
        ...
        # Extract binary bucket labels
        labels = [item['binary_bucket'] for item in labels_data['instructions']]
    ...
    # Create dataset
    dataset = BinaryInstructionDataset(all_instructions, all_labels, vocab, args.seq_len)
    ...
    print("\n" + "="*70)
    print("✓ Binary Bucket Probe Complete!")
    print("="*70)
