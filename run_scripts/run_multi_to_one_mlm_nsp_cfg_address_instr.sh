#!/bin/bash
# Run Multi-to-One training for: MLM + NSP-CFG + Address + Instruction-level segments
set -e
...
#!/bin/bash
# wrapper to run strupos/run_multi_to_one.sh with mode mlm_nsp_cfg_address_instr
set -e
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" >/dev/null && pwd)"
repo_root="$(cd "${script_dir}/.." >/dev/null && pwd)"

echo "running multi-to-one: mlm_nsp_cfg_address_instr"
cd "${repo_root}/strupos"
bash run_multi_to_one.sh MLM_NSP_CFG_ADDRESS_INSTR
